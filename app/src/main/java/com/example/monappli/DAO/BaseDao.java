...
import com.example.monappli.model.entities.BaseEntity;
...
public abstract class BaseDao<T extends BaseEntity> {
    ...
    public T getBestPlayer() {
        String sortOrder = "SCORE DESC"; // Tri décroissant sur la colonne du score
        SQLiteDatabase db = dbHelper.getReadableDatabase();
        ...
        Cursor cursor = db.query(
                ...
                null,
                sortOrder,
                "1" // Limite le résultat à 1 pour récupérer le score le plus grand
        );

        cursor.moveToFirst();
        T max = this.getEntity(cursor);
        cursor.close();
